...
const newPromise3 = new Promise((resolve, reject) => {
  setTimeout(() => {
    resolve({ username: "Aditya", email: "2101080022@hbtu.ac.in" }); //here objects ki jgh array wagera kuch bhi daal skte but u know objects hi aatw hai database se.
  }, 1000);
}).then((user) => {
...
// now we can do all this then and catch and finally using async and await...both could be used easily.We have to create promise first and then we wil able to access all
...
//await is put in front of every function which will cause us to wait...means promise will for sure make us wait but few things like data parsing to other format like parsing from string to json will also take some time and is we wont use await there it will get error
